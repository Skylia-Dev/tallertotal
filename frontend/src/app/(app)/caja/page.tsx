...
const tipoColor: Record<CajaMovimientoTipo, string> = {
  ...
  Cierre: "bg-gray-100 text-gray-700",
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Balance actual: <span className={`font-semibold ${resumen.balance >= 0 ? "text-green-600" : "text-red-600"}`}>${resumen.balance.toLocaleString("es-AR")}</span></p>
        </div>
                ...
                <TableBody>
                  {resumen.movimientos.map((m) => (
                    <TableRow key={m.id} className="hover:bg-gray-50">
                      <TableCell className="text-sm text-gray-600">{new Date(m.fecha).toLocaleString("es-AR")}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-sm text-gray-500">{m.observacion ?? "—"}</TableCell>
                      <TableCell className="text-sm text-gray-500">{m.createdByUsername}</TableCell>
